...
# 设置图片保存文件夹
app.config['UPLOAD_FOLDER'] = './img/out_image/'
 
# 设置图片返回的域名前缀
image_url = "https://118.31.3.198:8998/"
...
class YOLO(object):
    ...
    _defaults = {
        ...
        #--------------------------------------------------------------------------#
        "model_path"        : 'F:/Server_model/Spore/logs/best_epoch_weights.pth',
        "classes_path"      : 'F:/Server_model/Spore/model_data/voc_classes.txt',
        #---------------------------------------------------------------------#
        #   输入图片的大小，必须为32的倍数。
        #---------------------------------------------------------------------#
        "input_shape"       : [640, 640],
    ...
    def detect_image(self, image, crop = False, count = False):
        ...
        #   设置字体与边框厚度
        #---------------------------------------------------------#
        font        = ImageFont.truetype(font='F:/Server_model/Spore/model_data/simhei.ttf', size=np.floor(3e-2 * image.size[1] + 0.5).astype('int32'))
...
def uploads():
    if request.method == 'POST':
        ...
        if file and allowed_file(file.filename):
            ...
            return {"msg": "操作成功", "code": '200', "image_url": image_url + file_name}
        else:
            return "格式错误，仅支持jpg、png、jpeg格式文件"
        
   
if __name__ == "__main__":
    app.run(host="0.0.0.0", port=8091, debug=True)  # 项目入口